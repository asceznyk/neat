...
class Bird {
	...
	selectmove(NPil) {
	  ...
	  let action = this.model.predict(vis);
	...
	crossover(partner) {
	  for(let l = 1; l < this.model.layers.length; l++) {
	    if(partner.fitness > this.fitness) {
	      for(let i = 0; i < this.model.weightsandbiases['W'+l].selection.data.length; i++) {
	        if (i % 2 == 0) {
	          this.model.weightsandbiases['W'+l].selection.data[i] = partner.model.weightsandbiases['W'+l].selection.data[i];
	        }
	      }
	    }
	  }
